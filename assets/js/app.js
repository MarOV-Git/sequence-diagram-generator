...
/* ===== Handlers globales de drag (registrados una sola vez) ===== */
window.addEventListener("mousemove", function (e) {
  if (!activeDrag || !model) return;

  if (activeDrag.type === "header") {
    var p = activeDrag.p;
    var dx = e.clientX - activeDrag.startClientX;
    var newX = activeDrag.startPx + dx;
    ...
    if (idx > 0) {
      ...
      minX = left.x + left.headW + minGapBetween(left, p) - p.headW;
    }
    if (idx < model.participants.length - 1) {
      var right = model.participants[idx + 1];
      maxX = right.x - minGapBetween(p, right);
    }
    p.x = Math.max(minX, Math.min(maxX, newX));
    p.centerX = p.x + p.headW / 2;
    scheduleRender();

  } else if (activeDrag.type === "self") {
    var m = activeDrag.m;
    var dy = e.clientY - activeDrag.startClientY;
    if (activeDrag.resizing) {
      m._loopH = clamp(activeDrag.startLoopH + dy, DIAGRAM_CONFIG.selfLoop.MIN_H, DIAGRAM_CONFIG.selfLoop.MAX_H);
    } else {
      var minY = (m === _firstMsg) ? FIRST_MSG_MIN_Y : (DIAGRAM_CONFIG.layout.TOP_PAD + FIRST_OFFSET);
      var maxY = DIAGRAM_CONFIG.layout.TOP_PAD + DIAGRAM_CONFIG.layout.LIFELINE_H - (m._loopH || DIAGRAM_CONFIG.selfLoop.DEFAULT_H) - 6;
      m.y = clamp(activeDrag.startMsgY + dy, minY, maxY);
    }
    scheduleRender();

  } else if (activeDrag.type === "normal") {
    var m = activeDrag.m;
    var dy = e.clientY - activeDrag.startClientY;
    var minY = (m === _firstMsg) ? FIRST_MSG_MIN_Y : (DIAGRAM_CONFIG.layout.TOP_PAD + FIRST_OFFSET);
...
window.addEventListener("mouseup", function () {
  ...
  var type = activeDrag.type;
  var m = activeDrag.m;
  if (activeDrag.g) { activeDrag.g.classList.remove("dragging"); activeDrag.g.style.cursor = "grab"; }
  activeDrag = null;
  if (type === "header") { scheduleRender(); return; }
  try {
    cascadeShiftDown(m);
    var validMsgs = model.messages.filter(function (mm) { return mm._fromOk && mm._toOk; });
...
function renderFromModel() {
  ...
  model.participants.forEach(function (p) {
    ...
    g.addEventListener("mousedown", function (e) {
      activeDrag = { type: "header", p: p, g: g, startClientX: e.clientX, startPx: p.x };
  ...
  model.messages.forEach(function (m) {
    ...
    if (from.id === to.id) {
      ...
      function startSelfDrag(e) {
        activeDrag = { type: "self", m: m, startClientY: e.clientY, startMsgY: m.y, startLoopH: m._loopH || DIAGRAM_CONFIG.selfLoop.DEFAULT_H, resizing: e.shiftKey };
    ...
    if (m.ret) normalPath.setAttribute("stroke-dasharray", "6 4");
    ...
    function startNormalDrag(e) {
      activeDrag = { type: "normal", m: m, startClientY: e.clientY, startMsgY: m.y };
      e.preventDefault(); e.stopPropagation();
    }
...
document.getElementById("exportSvgBtn").onclick = function () {
  ...
  download(URL.createObjectURL(blob), "diagrama.svg");
};
